...
String calcularDataRelativa(unsigned long milliseconds) {
  ...
  // Aplica o deslocamento inicial de hora e minuto
  unsigned long startOffset = (START_HOUR * 3600UL) + (START_MINUTE * 60UL);
  unsigned long secondsSinceStart = totalSeconds + startOffset;

  // Calcula dias decorridos desde a data de referência
  ...
  // Função lambda para verificar se o ano é bissexto
  auto isLeapYear = [](int y) {
    return ((y % 4 == 0 && y % 100 != 0) || (y % 400 == 0));
  };

  // Dias corretos por mês (normal e bissexto)
  const int diasPorMesNormal[12]   = {31,28,31,30,31,30,31,31,30,31,30,31};
  const int diasPorMesBissexto[12] = {31,29,31,30,31,30,31,31,30,31,30,31};

  // Ajusta dia, mês e ano conforme necessário
  while (true) {
    const int* diasPorMes = isLeapYear(year) ? diasPorMesBissexto : diasPorMesNormal;
    if (day <= diasPorMes[month - 1]) break;
...
String formatarTempo(unsigned long milliseconds) {
  ...
  // Aplica deslocamento inicial de horário
  unsigned long startOffset = (START_HOUR * 3600UL) + (START_MINUTE * 60UL);
  totalSeconds += startOffset;

  unsigned long hours   = (totalSeconds / 3600UL) % 24;
  unsigned long minutes = (totalSeconds / 60UL) % 60;
